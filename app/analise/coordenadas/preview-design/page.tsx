            ...
            Object.entries(pointsConfig).forEach(([group, config]) => {
                for (let i = 1; i <= config.count; i++) {
                    ...
                    if (x !== undefined && y !== undefined) {
                        // Inverter Y se necessário? Geralmente MediaPipe Y cresce pra baixo (padrão canvas)
        ...
        for (let i = 0; i < data.length; i += sampleRate) {
            ...
            if (csvType === 'eyes_only') {
                Object.keys(pointsConfig).forEach(group => {
                    // Check just one point per group to estimate (e.g. RU1, RL1)
                    // Or loop properly if precision needed. Let's loop partial
                    for (let j = 1; j <= 3; j++) {
                ...
                // Check major landmarks (eyes, face contour)
                                        ...
                                        <div className="absolute top-4 left-4 flex gap-3 pointer-events-none">
                                            {csvType === 'eyes_only' && (
                                                <>
                                                    ...
                                                    <div className="flex items-center gap-2 px-3 py-1 bg-white/80 backdrop-blur rounded-full border border-teal-100 text-xs font-medium text-teal-700 shadow-sm">
